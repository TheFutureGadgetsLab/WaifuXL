...
export async function getTopTags(data) {
    ...
    const topDesc  = topK(flattened, 10, 0, 1000).map((i) => tags[i]);
    const topChars = topK(flattened, 10, 1000, 2000).map((i) => tags[i]);
    const rating   = topK(flattened, 1, 2000, 2003).map((i) => tags[i]);
